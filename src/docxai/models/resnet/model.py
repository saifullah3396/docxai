...
from docxai.models.resnet.definition import MODELS


@register_model(reg_name="resnet", task="image_classification")
class ResNetForImageClassification(BaseModuleForImageClassification):
    @dataclass
    ...
    def load_model(cls, model_name, num_labels, use_timm=False, pretrained=True, config=None, cache_dir=''):
        ...
        if num_labels is not None:
            return ResNetForImageClassification.update_classifier_for_labels(model, num_labels=num_labels)
        else:
            return model
